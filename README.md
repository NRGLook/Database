...
* Возможность у авторизированного / зарегистрированного пользователя:
   ...
   * отслеживания прогресса по выполненным задачам (это будет как банк (банк - это хранилище, которое будет создаваться при регистрировании нового пользователя, ), который будет пополняться после выполнения каждой цели)
   * добавления статьи (статья - это страница, которая будет привязываться ко дню для того, чтобы отследить впечатления за день)
...
7. **Впечатление за день (Articles)**
...
1. **Пользователь (Users)**
   - Пользователь представляет собой основную сущность, хранящую информацию о зарегистрированных пользователях.
   - Поля:
     - **ID**: Уникальный идентификатор пользователя (int, primary key, auto-increment).
...
2. **Роль (Roles)**
   - Роль определяет уровень доступа и права пользователя в системе.
   - Поля:
     - **ID**: Уникальный идентификатор роли (int, primary key, auto-increment).
...
3. **Действие пользователя (UserActions)**
   ...
   - Поля:
     - **ID**: Уникальный идентификатор действия (int, primary key, auto-increment).
...
4. **Журнал действий (AuditLog)**
   ...
   - Поля:
     - **ID**: Уникальный идентификатор записи в журнале (int, primary key, auto-increment).
...
5. **Задача (Tasks)**
   ...
   - Поля:
     - **ID**: Уникальный идентификатор задачи (int, primary key, auto-increment).
...
6. **Прогресс пользователя (UserProgress)**
   ...
   - Поля:
     - **ID**: Уникальный идентификатор записи о прогрессе задачи пользователя (int, primary key, auto-increment).
...
7. **Категория задачи (TaskCategory)**
   ...
   - Поля:
     - **ID**: Уникальный идентификатор категории задачи (int, primary key, auto-increment).
...
8. **Цель (Goals)**
   ...
   - Поля:
     - **ID**: Уникальный идентификатор цели (int, primary key, auto-increment).
...
9. **Прогресс цели (GoalProgress)**
   ...
   - Поля:
     - **ID**: Уникальный идентификатор записи о прогрессе цели пользователя (int, primary key, auto-increment).
     - **Дата**: Дата и время, когда был зарегистрирован прогресс (timestamp, NOT NULL).
...
10. **Статья (Articles)**
    ...
    - Поля:
      - **ID**: Уникальный идентификатор статьи (int, primary key, auto-increment).
